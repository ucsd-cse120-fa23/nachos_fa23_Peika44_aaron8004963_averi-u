...
public class VMProcess extends UserProcess {
  ...
  //   int amount = Math.min(length, (pageSize - vpnOff));
  ...
  //     if (phyAdd < 0 || phyAdd >= memory.length) {
  //       break;
  //     }
  ...
  //     // read from memory
  //     System.arraycopy(memory, phyAdd, data, next_offset, amount);
  ...
  // Lib.debug(dbgProcess, amount + " bytes to read from vpn " + vpn + " offset "
  // + vpnOff);
  ...
  // amount = Math.min(length - amount, pageSize);
  ...
  public int writeVirtualMemory(int vaddr, byte[] data, int offset, int length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (vpn < 0 || vpn >= pageTable.length || !entry.valid) break;

      int ppn = entry.ppn;
      if (!entry.readOnly) {

        int phyAdd = pageSize * ppn + Processor.offsetFromAddress(vaddr + i); 
        //Processor.offsetFromAddress represents how far into memory page the addr is 
